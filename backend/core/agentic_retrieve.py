...
def _get_prompt(key: str) -> str:
    """Load prompt from centralized registry with fallback."""
    from core.prompt_templates import PROMPTS
    return PROMPTS.get(key, "")


# Legacy aliases — now backed by the centralized registry
RETRIEVE_AGENT_SYSTEM_PROMPT = None  # lazy-loaded
ANSWER_AGENT_SYSTEM_PROMPT = None    # lazy-loaded


def _ensure_prompts():
    global RETRIEVE_AGENT_SYSTEM_PROMPT, ANSWER_AGENT_SYSTEM_PROMPT
    if RETRIEVE_AGENT_SYSTEM_PROMPT is None:
        RETRIEVE_AGENT_SYSTEM_PROMPT = _get_prompt("RETRIEVE_AGENT_SYSTEM")
    if ANSWER_AGENT_SYSTEM_PROMPT is None:
        ANSWER_AGENT_SYSTEM_PROMPT = _get_prompt("ANSWER_AGENT_SYSTEM")


# ---- Pydantic models for tool call argument validation ----

class GetDocumentArgs(BaseModel):
    """Arguments for get_document tool."""
    doc_id: str = Field(..., min_length=1, description="The document ID to get info for.")


class GetDocumentStructureArgs(BaseModel):
    """Arguments for get_document_structure tool."""
    doc_id: str = Field(..., min_length=1, description="The document ID to get structure for.")
...
class GetPageContentArgs(BaseModel):
    ...
    pages: str = Field(
        ...,
        min_length=1,
        description="Page range: '5-7', '3,8', or '12'.",
    )


# Map tool name → Pydantic model
...
def _validate_tool_args(func_name: str, raw_args: str) -> dict:
    """
    Parse and validate tool call arguments via Pydantic.
    Returns validated dict on success, or a dict with 'error' key on failure.
    """
    model_cls = _TOOL_ARG_MODELS.get(func_name)
    if model_cls is None:
        # Unknown tool — try raw parse
        try:
    ...
    try:
        ...
        logger.warning(f"Tool arg validation failed for {func_name}: {e}")
        return {"error": f"Argument validation failed for {func_name}: {e}"}
...
class AgenticRetriever:
    ...
    def _answer_via_llm(
        ...
        for round_num in range(self.max_tool_rounds):
            with tracer.span("agent.answer_round", round=round_num) as span:
                ...
                if message.tool_calls:
                    ...
                    for tc in message.tool_calls:
                        ...
                        if func:
                            try:
                                args = _validate_tool_args(func_name, tc.function.arguments)
                                if "error" in args:
        ...
        with tracer.span("agent.final_answer") as span:
    ...
    def _retrieve_via_llm(
        ...
        for round_num in range(self.max_tool_rounds):
            with tracer.span("agent.retrieve_round", round=round_num) as span:
                response = self._client.chat(
                    messages=messages,
                    tools=tool_definitions,
                    tool_choice="auto",
        ...
        context = self._format_context(fetched_content, doc_name_map, retrieval_summaries, query=query)
        sources = self._build_sources(raw_sources, doc_name_map)
    ...
    def _format_context(
        ...
        """Format fetched content with Cross-Encoder reranking + sandwich reordering.

        Industrial-grade approach:
          1. Split content into chunks
          2. Rerank by relevance using Cohere Rerank / heuristic
          3. Sandwich reorder: highest-scoring at edges, lowest in middle
          4. Summaries at BOTH edges (high attention positions)
        """
        parts = []
        ...
        if not parts and not summaries:
            return ""

        # Rerank content chunks by relevance if we have a query and enough chunks
        if query and len(parts) > 2:
            reranker = get_reranker()
            if reranker is not None:
                try:
                    reordered = sandwich_reorder_texts(query, parts, reranker)
                    parts = reordered
                    logger.info(f"[Agent] Reranked {len(parts)} context chunks")
                except Exception as e:
                    logger.warning(f"[Agent] Reranking failed, using original order: {e}")

        # Sandwich structure: summaries at BOTH edges, details in middle
        if summaries and len(parts) > 2:
            summary_text = "\n".join(summaries)
            recap = f"[Key findings recap — pay attention]:\n{summary_text}"
            return (
                f"## Agent Summary\n\n{summary_text}\n\n"
                f"---\n\n"
                + "\n\n---\n\n".join(parts)
